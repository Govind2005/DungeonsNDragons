...
public class GameController {
    ...
    public void handleAction(@Payload ActionMessage action, SimpMessageHeaderAccessor sha) {
        ...
        if (auth == null) {
            log.warn("Unauthenticated WS action rejected");
            return;
        }
    ...
    private void forwardToReferee(ActionMessage action, UUID matchId, UUID actorPlayerId) {
        try {
            ...
            refereeReq.put("targetPlayerId", action.getTargetPlayerId() != null && !action.getTargetPlayerId().isEmpty() ? action.getTargetPlayerId() : null);
