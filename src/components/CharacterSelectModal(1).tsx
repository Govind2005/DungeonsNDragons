  ...
  <div className="space-y-1">
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-1.5">
        ...
        <span className={`text-[10px] font-black tracking-widest uppercase ${active ? 'text-slate-400' : 'text-slate-600'}`}>
          {label}
        </span>
      </div>
            ...
            {characterClasses.map((cls) => {
              ...
              return (
                ...
                >
                  ...
                  {/* ── Character image: upper-half crop ── */}
                  <div className="relative h-52 overflow-hidden bg-black flex items-start justify-center">
                    ...
                    {/*
                      The image is shown full-size but the container clips it.
                      We position the image so only the top ~55% (torso + head) is visible.
                      object-top ensures the top of the image is always anchored.
                    */}
                    <img
                      src={CHARACTER_IMAGES[cls]}
                      alt={char.name}
                      className={`
                        relative z-10 w-full h-[190%] object-cover object-top
                        transition-all duration-500 drop-shadow-2xl
                        ${active ? 'scale-105' : 'scale-100'}
                      `}
                      style={{
                        objectPosition: 'top center',
                        // Pull image up slightly on hover for a "lean in" feel
                        transform: active ? 'scale(1.06) translateY(-2%)' : 'scale(1) translateY(0)',
                        transition: 'transform 0.4s ease',
                      }}
                    />

                    {/* Bottom fade into card body */}
                    <div className="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-slate-950 to-transparent z-20" />
                  </div>
