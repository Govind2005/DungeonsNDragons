...
export function LeaderboardScreen({ battleLogs, currentUserId, onBack }: LeaderboardScreenProps) {
  const [mounted, setMounted]     = useState(false);
  const [expanded, setExpanded]   = useState<string | null>(null);
  const [filter, setFilter]       = useState<'ALL' | 'WINS' | 'LOSSES'>('ALL');

  const wins    = battleLogs.filter(m => m.winnerTeam === 'blue').length;
  const losses  = battleLogs.length - wins;
  const winRate = battleLogs.length > 0 ? Math.round((wins / battleLogs.length) * 100) : 0;
  const shown   = filter === 'ALL' ? battleLogs : filter === 'WINS' ? battleLogs.filter(m => m.winnerTeam === 'blue') : battleLogs.filter(m => m.winnerTeam !== 'blue');
      ...
      <div className="relative z-10 pb-12">
        ...
        <div className="flex items-center justify-between px-8 py-5 border-b border-slate-800/50 bg-black/40 backdrop-blur-md">
          ...
          <div className="flex items-center gap-2 px-4 py-2 border border-slate-700/50 bg-slate-900/40">
            ...
            <span className="text-slate-400 text-[11px] tracking-widest">{battleLogs.length} MATCHES</span>
          </div>
        </div>
        ...
        <div className="px-8 py-8 max-w-5xl mx-auto space-y-5">
          ...
          <div className={`grid grid-cols-4 overflow-hidden border border-slate-800/50 transition-all duration-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            {[
              { v: battleLogs.length, l: 'TOTAL MATCHES', c: '#d9f99d' },
          ...
          <div className="space-y-4">
            {shown.map((m, idx) => {
              ...
              const blueWon = m.winnerTeam === 'blue';
              const winColor = blueWon ? '#60a5fa' : '#f87171';
              const winBg    = blueWon ? 'rgba(96,165,250,0.08)' : 'rgba(248,113,113,0.08)';
              const winBorder= blueWon ? 'rgba(96,165,250,0.35)' : 'rgba(248,113,113,0.35)';
              const bluePlayers = m.players.filter(p => p.team === 'blue');
              const redPlayers  = m.players.filter(p => p.team === 'red');
                  ...
                  <div className="px-5 py-4">
                    ...
                    <div className="flex items-center gap-4 mb-4">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-slate-500 text-[10px] tracking-widest">{timeAgo(m.timestamp)}</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <Users className="w-3 h-3 text-slate-600" />
                        <span className="text-slate-500 text-[10px] tracking-widest">{m.players.length} PLAYERS</span>
                      </div>
                    ...
                    <div className="grid grid-cols-[1fr_auto_1fr] gap-4 items-stretch">
                      ...
                      <div className="border border-slate-800/60 bg-slate-900/20 hover:bg-slate-900/40 transition-colors"
                        style={{ borderLeft: `3px solid rgba(96,165,250,${blueWon ? '0.8' : '0.3'})` }}>
                        <div className="flex items-center gap-2 px-3 py-2 border-b border-slate-800/40">
                          <div className="w-2 h-2 rounded-full" style={{ background: '#60a5fa', boxShadow: blueWon ? '0 0 8px rgba(96,165,250,0.8)' : 'none', opacity: blueWon ? 1 : 0.4 }} />
                          <span className="text-[10px] font-black tracking-widest" style={{ color: blueWon ? '#60a5fa' : '#475569' }}>TEAM BLUE</span>
                          {blueWon && <Trophy className="w-3 h-3 text-yellow-400 ml-auto" style={{ filter: 'drop-shadow(0 0 4px rgba(250,204,21,0.8))' }} />}
                        </div>
                        {bluePlayers.map((p, pi) => (
                          <div key={pi} className="flex items-center gap-3 px-3 py-2.5 border-b border-slate-800/20 last:border-0">
                            <div className="w-7 h-7 shrink-0 flex items-center justify-center text-[9px] font-black"
                              style={{ color: CLASS_COLOR[p.characterClass] || '#94a3b8', background: CLASS_BG[p.characterClass] || 'rgba(148,163,184,0.12)', border: `1px solid ${CLASS_COLOR[p.characterClass] || '#94a3b8'}40` }}>
                              {CLASS_SHORT[p.characterClass] || '??'}
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="text-white text-[11px] font-black tracking-wide truncate">{p.username}</div>
                              <div className="text-[9px] tracking-widest" style={{ color: CLASS_COLOR[p.characterClass] || '#94a3b8' }}>{(p.characterClass || '').toUpperCase()}</div>
                            </div>
                            <div className="flex items-center gap-2 shrink-0 text-right">
                              <div>
                                <div className="text-red-400 font-mono font-black text-[11px]">{p.damageDealt}</div>
                                <div className="text-[8px] text-slate-700 tracking-widest">DMG</div>
                              </div>
                              <div>
                                <div className="text-emerald-400 font-mono font-black text-[11px]">{p.healingDone}</div>
                              ...
                              <div className="px-1.5 py-1" style={{ background: 'rgba(163,230,53,0.07)', border: '1px solid rgba(163,230,53,0.2)' }}>
                                <div className="text-[11px] font-black font-mono" style={{ color: '#a3e635' }}>+{p.xpGained}</div>
                      ...
                      {/* Team Red */}
                      <div className="border border-slate-800/60 bg-slate-900/20 hover:bg-slate-900/40 transition-colors"
                        style={{ borderRight: `3px solid rgba(248,113,113,${!blueWon ? '0.8' : '0.3'})` }}>
                        <div className="flex items-center gap-2 px-3 py-2 border-b border-slate-800/40">
                          {!blueWon && <Trophy className="w-3 h-3 text-yellow-400" style={{ filter: 'drop-shadow(0 0 4px rgba(250,204,21,0.8))' }} />}
                          <div className="w-2 h-2 rounded-full ml-auto" style={{ background: '#f87171', boxShadow: !blueWon ? '0 0 8px rgba(248,113,113,0.8)' : 'none', opacity: !blueWon ? 1 : 0.4 }} />
                          <span className="text-[10px] font-black tracking-widest" style={{ color: !blueWon ? '#f87171' : '#475569' }}>TEAM RED</span>
                        </div>
                        {redPlayers.map((p, pi) => (
                      ...
                      <div className="flex items-center justify-between px-5 py-3">
                        <div className="flex items-center gap-5">
                          {[
                            ...
                            { icon: Users,  label: 'PLAYERS',  val: String(m.players.length) },
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-black text-[10px] tracking-widest" style={{ color: winColor }}>
                            {blueWon ? 'TEAM BLUE VICTORIOUS' : 'TEAM RED VICTORIOUS'}
                          </span>
                        </div>
                      </div>
                    </div>
